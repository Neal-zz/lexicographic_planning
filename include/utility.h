...
struct state_t {
    ...
    std::vector<neighbor_t> neighborList;
};
...
public:
    ...
    ParamServer() {
        ...
        nh.param<float>("roboat_planning/_mapResolution",    _mapResolution,    0.05);  // 0.05
        nh.param<float>("roboat_planning/_occuMapInflation", _occuMapInflation, 0.2);   // 0.2
        nh.param<float>("roboat_planning/_occuMapField",     _occuMapField,     0.4);   // 0.4

        nh.param<float>("roboat_planning/_sensorRangeLimitMin",    _sensorRangeLimitMin,    0.0);  // 0.0
        nh.param<float>("roboat_planning/_sensorRangeLimitMax",    _sensorRangeLimitMax,    5.0);  // 5.0
        nh.param<float>("roboat_planning/_sensorHeightLimitUpper", _sensorHeightLimitUpper, 0.5);  // 0.5
        nh.param<float>("roboat_planning/_sensorHeightLimitDown",  _sensorHeightLimitDown,  -0.2); // -0.2
        nh.param<float>("roboat_planning/_sensorCloudTimeout",     _sensorCloudTimeout,   5);      // 5

        nh.param<float>("roboat_planning/_samplingTipMargin", _samplingTipMargin, 0.5);  // 0.5
        nh.param<float>("roboat_planning/_samplingOutMargin", _samplingOutMargin, 0.2);  // 0.2
        ...
        _local_map_grid_num = round(_sensorRangeLimitMax * 4.0 / _mapResolution);  // 400
        _local_map_length = _sensorRangeLimitMax * 4.0;  // 20
    }

    /* 计算每个节点的朝向，补充 pathIn 的密度并平滑*/
    ...
    nav_msgs::Path fixPathDensity(nav_msgs::Path path) {
        ...
        const double margin = _pathResolution * 0.01;  // 0.1*0.01
        double dis = 0, ang = 0;
        double remaining = 0;
        int nPoints = 0;
